...
def read_file(args, file_, eval=False):
    dev_id = 0
    data = []
    with io.open(file_, encoding='utf-8') as fp:
        for line in fp:
            ...
            if eval:
                dev_id += 1 
                data.append((src, 0, dev_id))
                dev_id += 1 
                data.append((tgt, 1, dev_id))
            else:
                data.append((src, 0))
                data.append((tgt, 1))
...
def read_train_test_dev(args):
    dev_X = read_file(args, args.dev, eval=True)
    test_X = read_file(args, args.test, eval=True)
...
def eval(tagger, dev, ofp=None):
    ...
    sents = [s for s,l,o in sorted(sents, key=lambda x:x[2])]  # get back original dev set sentence order 
    for i,(src_sc, tgt_sc) in enumerate(zip(tagged[::2], tagged[1::2])):
        if src_sc < tgt_sc:
            op = 'True'
            good += 1
        else:
            op = 'False'
            bad += 1
        #print('\t'.join([' '.join(sents[i]), ' '.join(sents[i+1]), op]))
    print(good/(good+bad))
    sys.stdout.flush()
...
def main():
    ...
    if args.load_model:
        ...
        test = read_file(args, args.test, eval=True)
